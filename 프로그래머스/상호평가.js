...
// 0번 학생이 받은 점수는 0번 열에 담긴 [100, 50, 47, 61, 24]입니다. 자기 자신을 평가한 100점은 
// 자신이 받은 점수 중에서 유일한 최고점이므로, 평균을 구할 때 제외합니다.
...
function solution(scores) {
  ...
  let answer = '';
  let score = new Array(scores.length).fill(0);
  
  for (let i=0; i<scores.legnth; i++) {
      let lst = scores[i];
      let sum = 0;
      ...
      let count = lst.filter(c => c == lst[i]).length;
      if ((lst[i] == Math.max(...lst) || Math.min(...lst)) && count == 1) {
          let avg = (sum - lst[i])/(lst.length - 1);
          score[i] = avg;
      } else {
